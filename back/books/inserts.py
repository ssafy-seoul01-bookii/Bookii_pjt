        ...
        messages=[
            {
                ...
                카테고리인 old_category를 입력 받은 후, 소설/시/희곡, 경제/경영, 자기계발, 인문/교양, 과학, 취미/실용, 어린이/청소년, 종교/철학
                이 8가지 카테고리 중 입력 받은 카테고리와 가장 유사한 한 가지를 선택해서 문자열 데이터 타입으로 출력하시오.
                """
            },
            {
                "role": "user",
        ...
        messages=[
            {
                ...
                book_title을 입력 받으면, 성장, 감정, 사랑, 우정, 자기이해, 동기부여, 리더십, 창의성, 습관, 돈,
                시간관리, 커리어, 심리, 철학, 역사, 문화, 여행, 요리, 건강, 과학기술,
                AI, 환경, 미래, 교육, 감성, 창업, 자연, 가족, 명상, 스토리텔링,
                중 해당 도서에 관련된 2개 ~ 5개 사이의 키워드를 해시태그 형식으로 뽑으시오.
                그리고 키워드에 소설, 희곡, 에세이와 같은 도서 카테고리는 제외해서 선정하시오.
                인스타그램의 해시태그를 활용하면 좋을 것 같습니다.
                키워드를 아래의 출력 예시를 참고해서 #을 제거한 문자열로 출력하시오.
                출력 예시:
                키워드1,키워드2
                """
            },
...
def insert_book_category(new_category, book):
    if new_category in category_items:
        category = Category.objects.get(name=new_category,)
        Book_category.objects.create(book=book, category=category,)

def insert_book_keyword(new_keyword_list, book):
    for new_keyword in new_keyword_list:
        if new_keyword.strip() in keyword_items:
            keyword = Keyword.objects.get(name=new_keyword.strip(),)
            Book_keyword.objects.create(book=book, keyword=keyword,)
